...
import Antenna from '../../../../assets/qos.svg?react';
...
export const OverviewMap = ({
    ...
    const { mapQoSState, updateMapQoSState } = useMapCoordinates(); // Use context
    const [popup, setPopup] = useState<MarkerData | null>(null);
    const mapRef = useRef<MapRef>(null);
    const previousSelectedCellName = useRef('');
    const previousSelectedCellLatitude = useRef(MapConfig.initialLatitudeQoS);
    const previousSelectedCellLongitude = useRef(MapConfig.initialLongitudeQoS);
    const [clusters, setClusters] = useState<Cluster[]>([]);
    const [bounds, setBounds] = useState<BBox>(MapConfig.initialBounds);
    const [zoom, setZoom] = useState<number>(MapConfig.initialZoomQoS);
    ...
    return (
        <Box sx={styles.overviewMapWrapper}>
            <Map
                initialViewState={{
                    latitude: mapQoSState.coordinates.latitude,
                    longitude: mapQoSState.coordinates.longitude,
                    zoom: mapQoSState.zoom,
                    bearing: 0,
                    pitch: 0,
                }}
                ref={mapRef}
                        ...
                        return (
                            ...
                            >
                                <SvgIcon sx={styles.cluster(isActiveInCluster)}>
                                    <Antenna fill="#a9a9a9" stroke="white" />
                                </SvgIcon>
